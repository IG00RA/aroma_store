
import React from 'react';
import { motion } from 'framer-motion';
import { Star, Leaf, Shield } from 'lucide-react';

type FeatureCardProps = {
  icon: React.ReactNode;
  title: string;
  description: string;
  index: number;
};

const FeatureCard = ({ icon, title, description, index }: FeatureCardProps) => {
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: index * 0.2 }}
      viewport={{ once: true, margin: "-100px" }}
      whileHover={{ scale: 1.03, y: -5 }}
      className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-100 group"
    >
      <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-product-beige/70 to-product-beige/20 flex items-center justify-center text-product mb-6 shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-110">
        {icon}
      </div>
      <h3 className="text-2xl font-semibold mb-3 text-product-dark group-hover:text-product transition-colors duration-300">{title}</h3>
      <p className="text-gray-600 leading-relaxed">{description}</p>
    </motion.div>
  );
};

const FeaturesSection = () => {
  return (
    <section id="features" className="relative bg-gradient-to-t from-gray-50/80 to-white py-24">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-50"></div>
      <div className="container-section relative z-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          viewport={{ once: true, margin: "-100px" }}
          className="text-center max-w-3xl mx-auto mb-16"
        >
          <h2 className="text-3xl md:text-4xl font-bold text-product-dark">
            Почему наш держатель для благовоний особенный
          </h2>
          <p className="mt-4 text-gray-600 text-lg">
            Идеальное сочетание эстетики, функциональности и качества
          </p>
        </motion.div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          <FeatureCard
            index={0}
            icon={<Star className="h-7 w-7" />}
            title="Современный дизайн"
            description="Минималистичный стиль, который украсит любой интерьер и станет акцентным элементом вашего пространства."
          />
          <FeatureCard
            index={1}
            icon={<Leaf className="h-7 w-7" />}
            title="Экологичность"
            description="Безопасный PLA-пластик, производство с бережным отношением к природе и заботой об окружающей среде."
          />
          <FeatureCard
            index={2}
            icon={<Shield className="h-7 w-7" />}
            title="Продуманная эргономика"
            description="Удобство использования, надежная конструкция и простота в уходе для вашего комфорта каждый день."
          />
        </div>
      </div>
    </section>
  );
};

export default FeaturesSection;
