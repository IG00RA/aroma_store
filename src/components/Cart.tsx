...
import { ShoppingCart, X, Plus, Minus, Trash2, Phone, Mail, Flag, CreditCard, Truck, MapPin } from 'lucide-react';
...
const Cart = () => {
  ...
  const [step, setStep] = useState<'cart' | 'checkout'>(items.length > 0 ? 'cart' : 'checkout');
  ...
  useEffect(() => {
    ...
    if (savedIntegration) {
      try {
        ...
        console.error('Failed to parse integration settings from localStorage', e);
      }
    }
  ...
  const handleFinalCheckout = () => {
    ...
    localStorage.setItem('lastOrder', JSON.stringify({
      ...orderData,
      paymentDetails: checkoutData.paymentMethod === 'prepayment' ? paymentDetails : null,
      checkoutData: checkoutData // Keep the checkout data for the thank you page
    }));

    toast({
      title: "Заказ оформлен",
      description: `Спасибо за заказ, ${checkoutData.firstName}! Номер вашего заказа: ${orderId}`,
    });
  ...
  const isFormValid = () => {
    return (
      ...
      (checkoutData.deliveryService === '' || checkoutData.postalOffice.trim() !== '') &&
    ...
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative">
          ...
          {totalItems > 0 && (
            <Badge variant="destructive" className="absolute -top-2 -right-2 px-[0.4rem] min-w-[1.2rem] h-[1.2rem] flex items-center justify-center">
              {totalItems}
            </Badge>
          )}
        </Button>
      ...
      <SheetContent className="sm:max-w-md">
        <SheetHeader>
          <SheetTitle>{step === 'cart' ? 'Корзина' : 'Оформление заказа'}</SheetTitle>
        </SheetHeader>
        
        <div className="flex flex-col h-full">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center flex-grow py-12 text-center">
              <ShoppingCart className="h-16 w-16 text-muted-foreground mb-4" strokeWidth={1} />
              ...
              {step === 'cart' ? (
                <>
                  ...
                  <div className="py-4 space-y-4 border-t mt-auto">
                    ...
                    <Button onClick={handleProceedToCheckout} className="w-full btn-primary">
                      Оформить заказ
                    </Button>
                    <Button 
                      variant="outline" 
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select 
                          ...
                          onValueChange={(value) => handleCheckoutDataChange('country', value)}
                      ...
                      <div className="space-y-2">
                        ...
                        <Select 
                          ...
                          onValueChange={(value) => handleCheckoutDataChange('deliveryService', value)}
                        >
                          <SelectTrigger id="deliveryService" className="w-full">
                      ...
                      {checkoutData.deliveryService && (
                        <div className="space-y-2">
                          <Label htmlFor="postalOffice">Номер почтового отделения</Label>
                          <div className="relative">
                            <Input
                              id="postalOffice"
                              value={checkoutData.postalOffice}
                              onChange={(e) => handleCheckoutDataChange('postalOffice', e.target.value)}
                              placeholder="Укажите номер отделения"
                              className="pr-10"
                            />
                          ...
                          <p className="text-sm text-muted-foreground">
                            {checkoutData.deliveryService === 'novaposhta' 
                              ? "Укажите номер отделения Новой Почты для доставки"
                              : "Укажите номер отделения Укрпочты для доставки"}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        ...
                        <RadioGroup 
                          ...
                          onValueChange={(value) => handleCheckoutDataChange('paymentMethod', value)}
                          className="flex flex-col space-y-1"
                        >
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="prepayment" id="prepayment" />
                            <Label htmlFor="prepayment" className="flex items-center">
                      ...
                      {checkoutData.paymentMethod === 'prepayment' && (
                        <Alert className="bg-blue-50 border-blue-200">
                          ...
                          <AlertDescription className="text-sm text-blue-800">
                            <p className="font-medium">Информация об оплате:</p>
                            <p>Номер заказа: <strong>{orderId}</strong></p>
                            <p>После оформления заказа, пожалуйста, отправьте оплату на следующие реквизиты:</p>
                            <div className="mt-2 space-y-1">
                              <p>IBAN: <strong>{paymentDetails.iban}</strong></p>
                              <p>Карта: <strong>{paymentDetails.cardNumber}</strong></p>
                              <p>Банк: <strong>{paymentDetails.bankName}</strong></p>
                              <p>Получатель: <strong>{paymentDetails.recipientName}</strong></p>
                            </div>
                            <p className="mt-2">После оплаты, пожалуйста, отправьте подтверждение платежа через выбранный вами мессенджер.</p>
                          </AlertDescription>
                        </Alert>
                      )}
                      ...
                      <div className="space-y-3">
                        ...
                        <RadioGroup 
                          ...
                          onValueChange={(value) => handleCheckoutDataChange('contactMethod', value)}
                          className="flex flex-col space-y-1"
                        >
                          <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Label htmlFor="viber" className="flex items-center">
                              <Phone className="h-4 w-4 mr-2" />
                              Viber
                            </Label>
                          </div>
