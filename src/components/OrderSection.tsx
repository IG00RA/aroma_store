...
const OrderSection = () => {
  ...
  const [productName, setProductName] = useState("Держатель для ароматических палочек");
  ...
  const [deliveryInfo, setDeliveryInfo] = useState("Доставка по всей Украине. Срок доставки: 3-7 рабочих дней");
  const [guaranteeInfo, setGuaranteeInfo] = useState("Гарантия возврата денег в течение 30 дней");
  ...
  const getColorClass = (colorValue: string) => {
    switch (colorValue) {
      case 'white': return 'bg-product-white border border-gray-200';
      case 'black': return 'bg-product-black';
      case 'blue': return 'bg-blue-300';
      case 'beige': return 'bg-product-beige';
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-8">
              ...
              <p className="text-3xl font-bold text-product mb-6">{formatPrice(productPrice)}</p>
              
              <div className="space-y-6">
                {/* Color Selection */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Выберите цвет</label>
                      ...
                      {colors.map((color) => (
                        <SelectItem key={color.value} value={color.value}>
                          <div className="flex items-center">
                            <span className={`h-4 w-4 rounded-full mr-2 ${getColorClass(color.value)}`}></span>
                            {color.label}
                          </div>
                        </SelectItem>
                      ))}
